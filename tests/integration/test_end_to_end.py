...
print(json.dumps({"type": "result", "is_error": False, "result": "implemented",
                  "duration_ms": 10, "total_cost_usd": 0.01}), flush=True)
sys.exit(0)
"""
...
@pytest.mark.skipif(sys.platform != "win32", reason="batch stub is Windows-only")
def test_full_run_moves_phases_commits_and_archives(
    ...
    commit_dir.mkdir()
    monkeypatch.setenv("PLAN_IMPLEMENTER_CLAUDE", str(fake_claude))
    # Each commit drops one uniquely named marker file, so counting them proves both ran.
    monkeypatch.setenv(
        "PLAN_IMPLEMENTER_COMMIT",
...
def test_failing_claude_stops_before_moving_or_committing(
    ...
    launcher.write_text(f'@echo off\r\n"{sys.executable}" "{script}"\r\n', encoding="utf-8")
    monkeypatch.setenv("PLAN_IMPLEMENTER_CLAUDE", str(launcher))
    monkeypatch.setenv("PLAN_IMPLEMENTER_COMMIT", "")
